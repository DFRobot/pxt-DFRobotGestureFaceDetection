...
let REG_GFD_ADDR = 0x00  ///< Device address register
let REG_GFD_BAUDRATE = 0x01  ///< Baud rate configuration register
let REG_GFD_VERIFY_AND_STOP = 0x02  ///< Parity and stop bits configuration register
let REG_GFD_FACE_THRESHOLD = 0x03  ///< Face detection threshold, X coordinate
let REG_GFD_FACE_SCORE_THRESHOLD = 0x04  ///< Face score threshold
let REG_GFD_GESTURE_SCORE_THRESHOLD = 0x05 ///< Gesture score threshold

// GestureFaceDetection Data Register Addresses
let REG_GFD_PID = 0x00  ///< Product ID register
let REG_GFD_VID = 0x01  ///< Vendor ID register
let REG_GFD_HW_VERSION = 0x02  ///< Hardware version register
let REG_GFD_SW_VERSION = 0x03  ///< Software version register
let REG_GFD_FACE_NUMBER = 0x04  ///< Number of detected faces
let REG_GFD_FACE_LOCATION_X = 0x05  ///< Face X coordinate
let REG_GFD_FACE_LOCATION_Y = 0x06  ///< Face Y coordinate
let REG_GFD_FACE_SCORE = 0x07  ///< Face score
let REG_GFD_GESTURE_TYPE = 0x08  ///< Gesture type
let REG_GFD_GESTURE_SCORE = 0x09  ///< Gesture score
...
namespace DFRobot_GestureFaceDetection {
    ...
    export function functionconfigUart(baud: BaudConfig, parity: ParityConfig, stopBit: Stopbits): number {
        ...
        writeIHoldingReg(REG_GFD_BAUDRATE, baud)
        writeIHoldingReg(REG_GFD_VERIFY_AND_STOP, verifyAndStop)
        return 1
    }
    /**
    ...
    export function getGestureType(): number {
        let value = reaInputdReg(REG_GFD_GESTURE_TYPE)
        if (value > 10) value = 0
        return value
    }
